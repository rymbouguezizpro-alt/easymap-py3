...
function cr_analysis {
	...
	# Run vcf filter, selecting snps in the candidate region defined by map-mutation.py, with an alelic frequence > 0.8 and corresponding to EMS mutations # SDL V2 Update - Manages indels
	{
		...
		python3 $location/scripts_snp/variants-filter.py -a $f1/F2_control_comparison.va -b $f1/final_variants.va -step 2 -cand_reg_file $f1/map_info.txt -af_min 0.8 -mut_type $exp_mut_type  2>> $my_log_file
		python3 $location/scripts_snp/variants-filter.py -a $f1/F2_control_comparison_raw.va -b $f1/final_indels.va -step 4 -cand_reg_file $f1/map_info.txt -af_min 0.8 -mut_type $exp_mut_type  2>> $my_log_file
		python3 $location/scripts_snp/variants-filter.py -a $f1/F2_control_comparison_raw.va -b $f1/final_indels_total.va -step 5 -cand_reg_file $f1/map_info.txt -af_min 0.8 -mut_type all  2>> $my_log_file
	...
	# Create input for varanalyzer and run varanalyzer.py (one file for the candidate region and one for the whole genome)
	{
		...
		python3 $location/scripts_snp/snp-to-varanalyzer.py -a $f1/final_indels.va -b $f1/indel-to-varanalyzer.txt  2>> $my_log_file
		python3 $location/scripts_snp/snp-to-varanalyzer.py -a $f1/final_indels_total.va -b $f1/indel-to-varanalyzer-total.txt  2>> $my_log_file
	...
	result_extend_snp_info=`python3 $location/scripts_snp/extend-snp-variants-info.py  --variants $f1/primer_generation_output.txt --snp-info $f1/variants-to-varanalyzer.txt --project-name $project_name --map-info $f1/map_info.txt --output-file $f3/candidate_variants.txt --region CR 2>> $my_log_file`
	result_extend_snp_info=`python3 $location/scripts_snp/extend-snp-variants-info.py  --variants $f1/primer_generation_output_total.txt --snp-info $f1/variants-to-varanalyzer-total.txt --project-name $project_name --map-info $f1/map_info.txt --output-file $f3/candidate_variants_total.txt --region total 2>> $my_log_file`
	...
	{
		python3 $location/graphic_output/report.py -files_dir $f3 -variants $f3/candidate_variants.txt -c_format $control_format -log $f2/log.log -output_html $f3/report.html -project $project_name -mut_type $my_mut  2>> $my_log_file
...
then
	...
	{
		python3 $location/scripts_snp/variants-filter.py -a $f1/control_filtered.va -b $f1/control_filtered2.va -step 3 -fasta $f1/$my_gs -in_format $control_format -af_min 0.75  2>> $my_log_file
