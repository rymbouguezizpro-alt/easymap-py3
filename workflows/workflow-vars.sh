...
function get_control_va { 
	...
	{
		python3 $location/scripts_snp/variants-filter.py -a $f1/control_raw.va -b $f1/control_filtered.va -step 3 -fasta $f1/$my_gs -mut_type $exp_mut_type -dp_min $dp_min  -qual_min $problemSample_snpQualityTheshold  2>> $my_log_file
...
function get_control_va_from_vcf { 
	...
	{
		python3 $location/scripts_snp/variants-filter.py -in_format "vcf" -a $f1/control_raw.va -b $f1/control_filtered.va -step 3 -fasta $f1/$my_gs  -mut_type $exp_mut_type  2>> $my_log_file
...
function get_problem_va_from_vcf { 
	#Groom vcf
	{
		python3 $location/scripts_snp/vcf-groomer.py -a $my_rd -b $f1/F2_raw.va -step vcf_raw 2>> $my_log_file
	...
	{
		python3 $location/scripts_snp/variants-filter.py -in_format "vcf" -a $f1/F2_raw.va -b $f1/F2_filtered.va -step 3 -fasta $f1/$my_gs   -mut_type $exp_mut_type 2>> $my_log_file
...
# Get INDEL lists
{
	...
	python3 $location/scripts_snp/variants-filter.py -in_format $in_format -a $f1/F2_control_comparison_raw.va -b $f1/indels_total.va -step 5 -cand_reg_file $f1/candidate_region.txt -af_min 0.8 -mut_type $exp_mut_type  2>> $my_log_file
...
{
	python3 $location/scripts_snp/snp-to-varanalyzer.py -a $f1/F2_control_comparison.va -b $f1/snp-to-varanalyzer.txt  2>> $my_log_file
...
{ 
	...
	python3 $location/graphic_output/report.py  -log $my_log_file -t_format $problem_format -c_format $control_format -output_html $f3/report.html -project $project_name -mut_type vars -files_dir $f3 -cand_reg_file $f1/candidate_region.txt -variants $f3/candidate_variants.txt 2>> $my_log_file
