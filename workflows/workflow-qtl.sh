...
function get_problem_va {  
	...
	dp_min=3
	{
		python3 $location/scripts_snp/variants-filter.py -a $f1/F2_raw.va -b $f1/F2_filtered.va -step 3 -fasta $f1/$my_gs -dp_min $dp_min -qual_min $problemSample_snpQualityTheshold -mut_type $exp_mut_type  2>> $my_log_file
...
function get_control_va { 
	...
	{
		python3 $location/scripts_snp/variants-filter.py -a $f1/control_raw.va -b $f1/control_filtered.va -step 3 -fasta $f1/$my_gs -dp_min $dp_min -mut_type $exp_mut_type  2>> $my_log_file
...
{
	python3 $location/scripts_snp/af-comparison.py -mode qtl -f2_mut $f1/F2_filtered.va -f2_wt $f1/control_raw.va -out_test $f1/all_test_variants.va -out_qtl $f1/all_intersect_variants.va -out $f1/F2_control_comparison.va -f_input $f1/$my_gs  2>> $my_log_file 
...
# Run QTL mapping 
{
	...
	python3 $location/scripts_snp/qtl-mapping.py -in $f1/F2_control_comparison.va -mapqual $problemSample_snpQualityTheshold -out $f1/mapping_info.txt -out2 $f1/candidate_region.txt -width $wd_width -step $wd_step -f_input $f1/$my_gs	-cand_interval $interval_width 2>> $my_log_file
...
{
	python3 $location/scripts_snp/variants-filter.py -a $f1/all_test_variants.va -b $f1/test_variants_qtl.va -step 2 -cand_reg_file $f1/candidate_region.txt -mut_type $exp_mut_type  2>> $my_log_file
} || {
...
# Get INDEL lists
{
	python3 $location/scripts_snp/af-comparison.py -mode qtl -f2_mut $f1/F2_raw.va -f2_wt $f1/control_raw.va -out_test $f1/all_raw_test_variants.va -out_qtl $f1/all_raw_candidate_variants.va -out $f1/F2_control_comparison_raw.va -f_input $f1/$my_gs  2>> $my_log_file 
	python3 $location/scripts_snp/variants-filter.py -a $f1/all_raw_test_variants.va -b $f1/indels.va -step 4 -cand_reg_file $f1/candidate_region.txt -mut_type all  2>> $my_log_file
	python3 $location/scripts_snp/variants-filter.py -a $f1/all_raw_test_variants.va -b $f1/indels_total.va -step 5 -cand_reg_file $f1/candidate_region.txt -mut_type all  2>> $my_log_file
...
{
	python3 $location/scripts_snp/snp-to-varanalyzer.py -a $f1/test_variants_qtl.va -b $f1/snp-to-varanalyzer.txt  2>> $my_log_file
	python3 $location/scripts_snp/snp-to-varanalyzer.py -a $f1/all_test_variants.va -b $f1/snp-to-varanalyzer-total.txt  2>> $my_log_file		
...
# Varanalyzer
{
	touch $f1/varanalyzer_output_snp.txt ;          python3 $location/varanalyzer/varanalyzer.py  -thr $n_threads -itp snp -con $f1/$my_gs -gff $f0/$my_gff -var $f1/snp-to-varanalyzer.txt -rrl $my_rrl -pname $project_name -ann $f0/$my_ann -out $f1/varanalyzer_output_snp.txt 2>> $my_log_file
	touch $f1/varanalyzer_output_total_temp.txt  ;  python3 $location/varanalyzer/varanalyzer.py  -thr $n_threads -itp snp -con $f1/$my_gs -gff $f0/$my_gff -var $f1/snp-to-varanalyzer-total.txt -rrl $my_rrl -pname $project_name -ann $f0/$my_ann -out $f1/varanalyzer_output_total_temp.txt  2>> $my_log_file
...
result_extend_snp_info=`python3 $location/scripts_snp/extend-snp-variants-info.py  --variants $f1/primer_generation_output.txt --snp-info $f1/variants-to-varanalyzer.txt --project-name $project_name --map-info $f1/candidate_region.txt --output-file $f1/candidate_variants_temp.txt --region total 2>> $my_log_file`
result_extend_snp_info_2=`python3 $location/scripts_snp/extend-snp-variants-info.py  --variants $f1/primer_generation_output_total.txt --snp-info $f1/variants-to-varanalyzer-total.txt --project-name $project_name --map-info $f1/candidate_region.txt --output-file $f1/candidate_variants_total_temp.txt --region total 2>> $my_log_file`
...
# Run qtl-genes.py
{
	python3 $location/scripts_snp/qtl-genes.py -gff $f0/$my_gff -cr $f1/candidate_region.txt -out $f3/all_qtl_genes.txt -f_an $f0/$my_ann  2>> $my_log_file
...
{
	python3 $location/scripts_snp/extend-qtl-variants-info.py -cand $f1/candidate_variants_temp.txt -af-info $f1/all_raw_test_variants.va -out $f3/candidate_variants.txt  2>> $my_log_file
	python3 $location/scripts_snp/extend-qtl-variants-info.py -cand $f1/candidate_variants_total_temp.txt -af-info $f1/all_raw_test_variants.va -out $f3/candidate_variants_total.txt  2>> $my_log_file
...
{
	python3 $location/graphic_output/graphic-output.py -my_mut qtl -asnp $f1/F2_control_comparison.va -gff $f0/$my_gff  -iva $f1/varanalyzer_output.txt -rrl $my_rrl -pname $project_name  -bsnp $f1/$my_gs  -cr_file $f1/candidate_region.txt  -mut_type $exp_mut_type 2>> $my_log_file
...
{ 
	...
	python3 $location/graphic_output/report.py  -log $my_log_file  -output_html $f3/report.html -project $project_name -mut_type qtl -files_dir $f3 -cand_reg_file $f1/candidate_region.txt -variants $f3/candidate_variants.txt 2>> $my_log_file
